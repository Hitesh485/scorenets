#!/usr/bin/env python3
"""Deploy footer © 2026 ScoreNet space fix (boot.js only)."""
...
def find_pem() -> Path:
    for c in [Path(os.environ.get("TEMP", "")) / "sn_pem_try5" / "betting_production.pem"]:
        if c.is_file():
            return c
    pem_dir = Path(os.environ["TEMP"]) / f"sn_pem_sp_{STAMP}"
...
def main() -> None:
    ...
    assert "snFixCopyrightYearBrandSpace" in boot
    print("LOCAL_OK", VER)
...
assert "snFixCopyrightYearBrandSpace" in Path("/var/www/scorenet/brand/boot.js").read_text(encoding="utf-8", errors="ignore")
print("DEPLOY_OK", ver)
PY
rm -f '{remote_boot}'
echo REVERT_DIR $REV
"""
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "sn-sp", "Cache-Control": "no-cache"},
    )
    body = urllib.request.urlopen(req, timeout=40).read().decode("utf-8", "replace")
    assert "snFixCopyrightYearBrandSpace" in body
    print("LIVE_OK", VER)
