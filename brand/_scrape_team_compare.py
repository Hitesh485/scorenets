"""Scrape Sofascore Team comparison into a ScoreNet freeze shell.

Same pipeline as _scrape_ql_pages.py, plus:
- mobile / tablet / desktop viewports
- API JSON capture (api.sofascore.com + /api/v1)
- img.sofascore.com logos when possible
"""
...
OUT_DIR = ROOT / "football" / "team" / "compare"
API_DIR = OUT_DIR / "_raw_api"
OUT_META = OUT_DIR / "_sn_compare_scrape.json"
VER = "20260908cmp"

TARGET = (
    "https://www.sofascore.com/football/team/compare"
    "?ids=2695%2C2699&s_ids=95836%2C95836&ut_ids=23%2C23"
)
...
# Primary shell = mobile (match Overview CTA bug is phone-first)
PRIMARY_VIEW = "mobile"
...
def rebrand_and_finalize(html: str, page_flag: str) -> str:
    ...
    brand = (
        ...
        + f'<script src="/brand/live-ticker.js?v=20260905g"></script>'
...
def settle_page(page) -> None:
    ...
    try:
        page.wait_for_selector("text=Team comparison", timeout=20000)
    except Exception as e:
        print("wait Team comparison:", e)
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        for vp in VIEWPORTS:
            ...
            html = page.content()
            title = page.title()
            ok = "Team comparison" in html and "404" not in title
            ...
            view_results.append(
                {
                    ...
                    "has_general": "General" in html,
                    "has_attacking": "Attacking" in html,
                    "has_udinese": "Udinese" in html,
                }
            )
            print(
                "DONE",
    ...
    # also keep a generic _raw.html = primary
    ...
    html = rebrand_and_finalize(html, "football-team-compare")
    ...
    for v in view_results:
        print("VIEW", v["viewport"], "ok", v["ok"], "udinese", v["has_udinese"], "general", v["has_general"])
