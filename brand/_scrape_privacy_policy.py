"""Scrape Sofascore Privacy Policy into a ScoreNet freeze shell.
...
OUT_DIR = ROOT / "privacy-policy"
API_DIR = OUT_DIR / "_raw_api"
OUT_META = OUT_DIR / "_sn_privacy_scrape.json"
VER = "20260909pp"

TARGET = "https://www.sofascore.com/privacy-policy"
...
# Desktop freeze keeps full header ticker + sports navbar (Sofa parity)
...
def rebrand_and_finalize(html: str, page_flag: str) -> str:
    ...
    # Keep legal entity names accurate where needed, but brand the product UI
    html = html.replace("Sofascore", "ScoreNet").replace("SofaScore", "ScoreNet")
    html = html.replace("ScoreNetSans", "SofascoreSans")
    # Restore legal company references that should stay Sofa IT
    html = html.replace("ScoreNet's privacy policy", "ScoreNet's privacy policy")
    html = html.replace("https://www.ScoreNet.com", "https://scorenets.com")
    # DO NOT replace inside /assets/www.sofascore.com/ — only bare host names
    ...
    # Guard: if replace leaked into assets path, restore
    ...
    # Fix H1 after Sofascore→ScoreNet replace
    html = html.replace("SCORENET PRIVACY POLICY", "SCORENET PRIVACY POLICY")
    return html
...
def settle_page(page) -> None:
    ...
    try:
        page.wait_for_selector("text=PRIVACY POLICY", timeout=25000)
    except Exception as e:
        print("wait PRIVACY POLICY:", e)
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        for vp in VIEWPORTS:
            ...
            # Also pull live ticker feed snapshot explicitly
            ...
            html = page.content()
            title = page.title()
            ok = ("PRIVACY POLICY" in html.upper()) and ("Sofa IT" in html or "SOFASCORE PRIVACY" in html.upper() or "privacy policy" in html.lower())
            ...
            view_results.append(
                {
                    ...
                    "has_manage_consent": "MANAGE CONSENT" in html,
    ...
    html = rebrand_and_finalize(html, "privacy-policy")
