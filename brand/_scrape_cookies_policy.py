"""Scrape Sofascore Cookie Policy into a ScoreNet freeze shell.
...
OUT_DIR = ROOT / "cookies-policy"
API_DIR = OUT_DIR / "_raw_api"
OUT_META = OUT_DIR / "_sn_cookies_scrape.json"
VER = "20260909cp"

TARGET = "https://www.sofascore.com/cookies-policy"
...
# Desktop keeps full navbar ticker strip (mobile often collapses chips).
...
def rebrand_and_finalize(html: str, page_flag: str) -> str:
    ...
    # Protect local asset folder from domain rewrite
    html = html.replace("/assets/www.sofascore.com/", "/assets/__SN_ASSETS__/")
    ...
    # Fix policy website URL (screenshot bug)
    html = html.replace("https://www.sofascore.com/", "https://scorenets.com/")
    html = html.replace("https://www.sofascore.com", "https://scorenets.com")
    html = html.replace("/assets/__SN_ASSETS__/", "/assets/www.sofascore.com/")
...
def settle_page(page) -> None:
    ...
    try:
        page.wait_for_selector("text=COOKIE POLICY", timeout=25000)
    except Exception as e:
        print("wait COOKIE POLICY:", e)
    try:
        page.wait_for_selector("text=What are cookies?", timeout=15000)
    except Exception as e:
        print("wait What are cookies:", e)
    # Nudge ticker / lazy bits
    try:
        page.wait_for_selector("header a[data-id]", timeout=20000)
    except Exception as e:
        print("wait ticker chips:", e)
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        for vp in VIEWPORTS:
            ...
            # Explicit live ticker feeds
            for sport in ("1", "football"):
                try:
                    live_url = f"https://www.sofascore.com/api/v1/sport/{sport}/events/live"
                    r = page.request.get(live_url, timeout=20000)
                    if r.status == 200:
                        body = r.body()
                        dest = API_DIR / f"live__sport_{sport}_events_live.json"
                        dest.write_bytes(body)
                        api_saved.append(
                            {
                                "viewport": name,
                        ...
                        print("LIVE_API", sport, len(body))
                except Exception as e:
                    print("LIVE_API_FAIL", sport, e)

            # Also grab scheduled/today strip feed used by ticker chips if exposed
            for extra in (
                "https://www.sofascore.com/api/v1/event/newly-added-events",
                "https://www.sofascore.com/api/v1/sport/football/scheduled-events",
            ):
                try:
                    r = page.request.get(extra, timeout=20000)
                    if r.status == 200:
                        body = r.body()
                        dest = API_DIR / f"extra__{safe_api_name(extra)}"
                        if not dest.exists():
                            dest.write_bytes(body)
                        api_saved.append(
                            {
                        ...
                        print("EXTRA_API", extra, len(body))
                except Exception as e:
                    print("EXTRA_API_FAIL", extra, e)

            html = page.content()
            title = page.title()
            ok = ("COOKIE POLICY" in html.upper() or "Cookie Policy" in html) and (
                "What are cookies" in html or "WHAT ARE COOKIES" in html.upper()
            )
            ...
            # Ticker presence stats
            ticker_n = 0
            try:
                ticker_n = page.locator("header a[data-id]").count()
            except Exception:
                pass
            ...
            view_results.append(
                {
                    ...
                    "ticker_chips": ticker_n,
                    "has_what_are_cookies": "What are cookies" in html,
                    "has_sofa_url": "www.sofascore.com" in html,
                }
            )
            print(
                "DONE",
    ...
    if not primary_raw.exists():
        primary_raw = OUT_DIR / "_raw_desktop.html"
    ...
    html = rebrand_and_finalize(html, "cookies-policy")
