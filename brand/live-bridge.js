/* ScoreNet live-bridge: APIs + Next chunks + images + deep-route pageProps + media embeds */
(function () {
  ...
  // SofaScore client uses absolute https://www.sofascore.com/api/v1/... (incl. HEAD
  // for hasHighlights). From scorenets.com that often fails CORS/WAF — always same-origin
  // so SW (prod) or serve_spa.py (local) can proxy.
  function mapApiPath(pathname, search) {
    return location.origin + pathname + (search || "");
  }

  function isMediaEmbedSrc(src) {
    return /youtube\.com\/embed|youtube-nocookie\.com\/embed|youtu\.be\/|player\.vimeo\.com\/video|wsc-sports|blazesports|ott\.sofascore\.com/i.test(
      String(src || "")
    );
  }

  // Idempotent — never write attributes if already correct (avoids MO infinite loop).
  function fixMediaIframe(iframe) {
    if (!iframe || (iframe.tagName || "").toUpperCase() !== "IFRAME") return;
    if (iframe.getAttribute("data-sn-yt-ok") === "1") return;
    var src = iframe.getAttribute("src") || iframe.src || "";
    if (!isMediaEmbedSrc(src)) return;
    try {
      var want = "strict-origin-when-cross-origin";
      var cur = (iframe.getAttribute("referrerpolicy") || iframe.referrerPolicy || "").toLowerCase();
      if (cur !== want) {
        iframe.setAttribute("referrerpolicy", want);
        try {
          iframe.referrerPolicy = want;
        } catch (e0) {}
      }
      ...
      if (iframe.getAttribute("data-sn-ad-hide")) {
        iframe.removeAttribute("data-sn-ad-hide");
      }
      iframe.setAttribute("data-sn-yt-ok", "1");
    } catch (e) {}
  }
  ...
  function mapUrl(input) {
    try {
      ...
      // Absolute SofaScore JSON API → same-origin /api (SW) or api.sofascore.com
      if (
        (u.hostname === "api.sofascore.com" || u.hostname === "www.sofascore.com") &&
        u.pathname.indexOf("/api/") === 0
      ) {
        return mapApiPath(u.pathname, u.search);
      }
  ...
  function asyncGetJson(url) {
    ...
    return fetch(mapped, { credentials: "omit", mode: "cors", referrerPolicy: "no-referrer", cache: "no-store" }).then(
      function (r) {
  ...
  try {
    ...
    Element.prototype.setAttribute = function (name, value) {
      ...
      // SVG <image href> (Weekly Challenge league trophies etc.)
      if (tag === "image" && (n === "href" || n === "xlink:href")) {
      ...
      // YouTube Error 153: set referrer policy BEFORE src (no post-call fixMedia — avoids loop)
      if (tag === "iframe" && n === "src" && isMediaEmbedSrc(v)) {
        try {
          if ((this.getAttribute("referrerpolicy") || "").toLowerCase() !== "strict-origin-when-cross-origin") {
            _setAttr.call(this, "referrerpolicy", "strict-origin-when-cross-origin");
            try {
              this.referrerPolicy = "strict-origin-when-cross-origin";
            } catch (e4) {}
          }
          _setAttr.call(this, "data-sn-yt-ok", "1");
        } catch (e3) {}
      }
  ...
  function sweep(root) {
    try {
      ...
      (root || document)
        .querySelectorAll(
          'img[src*="/api/v1/"],source[srcset*="/api/v1/"],script[src*="/_next/"],image[href*="/api/v1/"],image[href*="sofascore.com"],[style*="unique-tournament"],[style*="/api/v1/"]'
        )
        .forEach(function (el) {
          ...
          if ((el.tagName || "").toLowerCase() === "image") {
            var ih = el.getAttribute("href") || el.getAttribute("xlink:href");
            if (ih) {
              var mapped = mapUrl(ih);
              if (mapped !== ih) {
                el.setAttribute("href", mapped);
                try {
                  el.setAttributeNS("http://www.w3.org/1999/xlink", "href", mapped);
                } catch (eImg) {}
              }
            }
          }
      ...
      // One-shot, idempotent YouTube iframe policy (not on every attribute mutation)
      (root || document).querySelectorAll("iframe[src]").forEach(fixMediaIframe);
    } catch (e) {}
  }

  // Patch iframe.src: policy before navigation only — do not call fixMediaIframe here
  try {
    var idesc = Object.getOwnPropertyDescriptor(HTMLIFrameElement.prototype, "src");
    if (idesc && idesc.set) {
      Object.defineProperty(HTMLIFrameElement.prototype, "src", {
        ...
        set: function (v) {
          try {
            if (isMediaEmbedSrc(v)) {
              if ((this.getAttribute("referrerpolicy") || "").toLowerCase() !== "strict-origin-when-cross-origin") {
                this.setAttribute("referrerpolicy", "strict-origin-when-cross-origin");
                try {
                  this.referrerPolicy = "strict-origin-when-cross-origin";
                } catch (e1) {}
              }
              this.setAttribute("data-sn-yt-ok", "1");
            }
          } catch (e) {}
  ...
  try {
    ...
    new MutationObserver(function (muts) {
      try {
        // Only on newly added nodes — NEVER on attribute changes (referrerpolicy/src loop)
        muts.forEach(function (m) {
          if (!m.addedNodes) return;
          m.addedNodes.forEach(function (n) {
            if (!n || n.nodeType !== 1) return;
            if (n.tagName === "IFRAME") fixMediaIframe(n);
            else if (n.querySelectorAll) n.querySelectorAll("iframe[src]").forEach(fixMediaIframe);
          });
        });
      } catch (e2) {}
      if (t) return;
